# /planning/infrastructure/repositories.py
...
class ThresholdRepository:
    """
    # Repository for managing the local CACHE of PotThreshold entities.
    # It translates between the domain entity and the database cache model.
    """

    @staticmethod
    def _to_entity(model: PotThresholdModel) -> PotThreshold:
        """
        # A private helper to map the Peewee model to the domain entity.
        """
        return PotThreshold(
            id=model.id,
    ...
    def get_for_device(device_id: str) -> PotThreshold | None:
        """
        # Tries to retrieve the cached thresholds for a device from the local DB.
        # Returns the domain entity if found, otherwise None.
        """
        try:
    ...
    def save(threshold: PotThreshold) -> PotThreshold:
        """
        # Saves or updates the thresholds for a device in the local cache.
        # This is called after a successful fetch from the cloud.
        """
        ...
        # If the record was not created, it means it already existed, so we update it.
        if not created:
